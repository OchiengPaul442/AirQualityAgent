...
async def create_new_session(db: Session = Depends(get_db)):
    ...
    from infrastructure.database.repository import create_session
...
async def get_session_messages(
    ...
    from infrastructure.database.repository import get_session_history
...
async def chat(
    ...
    **Example Flow:**
    ...
    try:
        ...
        from shared.monitoring.error_logger import get_error_logger
    ...
    async def generate_thoughts():
        ...
        try:
            ...
            async def process_and_respond_internal():
                ...
                try:
                    # Add small delay to ensure stream iteration starts first
                    ...
                    # Ensure stream is completed even if agent forgets
                    if stream and not stream._closed:
                        logger.warning("Agent didn't close stream, closing manually")
                        await stream.complete({
                            "status": "auto-completed",
                            "note": "Stream was not explicitly closed by agent"
                        })
                    ...
                    if stream and not stream._closed:
                        await stream.emit_error(str(e), recoverable=False)
                        await stream.complete({"status": "error", "error": str(e)})
                    raise
            ...
            try:
                ...
                # Log warning if no thoughts were emitted
                if thought_count == 0:
                    logger.warning("⚠️ No thoughts were emitted during processing - check agent implementation")
