...
class CostOptimizer:
    ...
    def track_token_usage(self, session_id: str, tokens_used: int, cost: float = 0.0) -> dict[str, Any]:
        """
    ...
    def should_use_cheaper_model(self, query: str) -> bool:
        ...
        simple_patterns = [
            "what is", "what's the", "current", "latest",
            "how is", "how's the", "show me", "get"
        ]
        
        # Complex queries that need advanced models
        complex_patterns = [
            "analyze", "compare", "explain why", "what if",
            "calculate", "predict", "recommend"
        ]
    ...
    def deduplicate_request(self, request_key: str) -> bool:
        ...
        expired_keys = [
            k for k, v in self.active_requests.items()
            if current_time - v["start_time"] > 30
        ]
