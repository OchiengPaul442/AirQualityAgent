...
class SessionContextManager:
    ...
    def __init__(self, max_contexts: int = 50, context_ttl: int = 3600):
        ...
        logger.info(
            f"SessionContextManager initialized (max_contexts={max_contexts}, ttl={context_ttl}s)"
        )
        ...
        if len(recent_messages) > 10:
            ...
            if topics:
                context["conversation_summary"] = (
                    f"Previous conversation covered: {', '.join(topics)}"
                )
                logger.debug(
                    f"Updated summary for session {session_id[:8]}: {context['conversation_summary']}"
                )
    ...
    def _cleanup_old_contexts(self) -> None:
        ...
        if len(self.session_contexts) > self.max_contexts:
            sorted_sessions = sorted(
                self.session_contexts.items(), key=lambda x: x[1].get("last_access", 0)
            )
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        """
        total_documents = sum(
            len(ctx.get("documents", [])) for ctx in self.session_contexts.values()
        )
